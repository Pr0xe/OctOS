...
{
	for (int i = 0; i < OCTOS_MAX_PROCESSES; i++) {
		if (processes[i]) {
			process_switch(processes[i]);
			return;
		}
	}
	panic("Kernel panic: No processes to switch\n");
}
...
int process_terminate(struct process *process)
{
	...
	//Unlink the process from process array
			     ...
			     struct command_argument *root_argument)
{
	...
	while (current) {
		char *argument_str =
			process_malloc(process, sizeof(current->argument));
		if (!argument_str) {
			res = -ENOMEM;
			goto out;
		}

		strncpy(argument_str, current->argument,
			sizeof(current->argument));
		argv[i] = argument_str;
		current = current->next;
		i++;
	}
...
void process_free(struct process *process, void *ptr)
{
	...
	process_unjoin_allocation(process, ptr);
	kfree(ptr);
}
...
int process_map_memory(struct process *process)
{
	...
	//map the stack
	paging_map_to(process->task->page_directory,
		      (void *)OCTOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END,
		      process->stack,
		      paging_align_address(process->stack +
					   OCTOS_USER_PROGRAM_STACK_SIZE),
		      PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL |
			      PAGING_IS_WRITABLE);
out:
	return res;
			  ...
			  int process_slot)
{
	...
	program_stack_ptr = kzalloc(OCTOS_USER_PROGRAM_STACK_SIZE);
