...
void task_next()
{
	...
	if (!next_task) {
		panic("Kernel panic: No more tasks\n");
	}
...
int task_init(struct task *task, struct process *process)
{
	...
	task->registers.ip = OCTOS_PROGRAM_VIRTUAL_ADDRESS;
	...
	task->registers.esp = OCTOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START;
...
void *task_virtual_address_to_physical(struct task *task, void *virtual_address)
{
	return paging_get_physical_address(
		task->page_directory->directory_entry, virtual_address);
}