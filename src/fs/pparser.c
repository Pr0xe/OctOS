...
{
	int len = strnlen(filename, OCTOS_MAX_PATH);
...
{
	char *result_path_part = kzalloc(OCTOS_MAX_PATH);
	int i = 0;
				   ...
				   const char *current_directory_path)
{
	...
	struct path_part *first_part =
		pathparser_parse_path_part(NULL, &tmp_path);
	if (!first_part) {
		goto out;
	}
	path_root->first = first_part;
	struct path_part *part =
		pathparser_parse_path_part(first_part, &tmp_path);
	while (part) {
