...
struct fat_private {
	...
	//Used to stream the file alloc table
					...
					uint32_t directory_start_sector)
{
	...
	if (diskstreamer_seek(stream, directory_start_pos) != OCTOS_ALL_OK) {
		res = -EIO;
		goto out;
	}
	while (1) {
			     ...
			     struct fat_directory *directory)
{
	...
	int root_dir_entries =
		fat_private->header.primary_header.root_dir_entries;
	int root_dir_size =
	...
	struct disk_stream *stream = fat_private->directory_stream;
	if (diskstreamer_seek(stream, fat16_sector_to_absolute(
					      disk, root_dir_sector_pos)) !=
	    OCTOS_ALL_OK) {
		res = -EIO;
		goto out;
	}

	if (diskstreamer_read(stream, dir, root_dir_size) != OCTOS_ALL_OK) {
		res = -EIO;
		goto out;
	}
...
int fat16_resolve(struct disk *disk)
{
	...
	if (diskstreamer_read(stream, &fat_private->header,
			      sizeof(fat_private->header)) != OCTOS_ALL_OK) {
		res = -EIO;
		goto out;
	}
...
{
	return (item->high_16_bits_first_cluster) |
...
/* Gets correct cluster to use based on the starting cluster and the offset*/
					       ...
					       struct fat_directory_item *item)
{
	...
	directory->total = total_items;
	int directory_size =
		directory->total * sizeof(struct fat_directory_item);
				      ...
				      struct fat_directory_item *item)
{
	...
	f_item->item = fat16_clone_directory_item(
					      ...
					      const char *name)
{
	...
	char tmp_filename[OCTOS_MAX_PATH];
	for (int i = 0; i < directory->total; i++) {
		fat16_get_full_relative_filename(&directory->item[i],
						 tmp_filename,
						 sizeof(tmp_filename));
		if (istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0) {
			f_item = fat16_new_fat_item_for_directory_item(
				disk, &directory->item[i]);
		}
	}
	return f_item;
}
