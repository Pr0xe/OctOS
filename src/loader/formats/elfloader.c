...
{
	return memcmp(buffer, (void *)elf_signature, sizeof(elf_signature)) ==
	       0;
}

static bool elf_valid_class(struct elf_header *header)
{
	//we only support 32 bits
...
static bool elf_is_executable(struct elf_header *header)
{
	return header->e_type == ET_EXEC &&
	       header->e_entry >= OCTOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
{
	return (char *)header +
	       elf_section(header, header->e_shstrndx)->sh_offset;
}
...
int elf_process_phdr_pt_load(struct elf_file *elf_file, struct elf32_phdr *phdr)
{
	if (elf_file->virtual_base_address >= (void *)phdr->p_vaddr ||
		...
		elf_file->physical_base_address =
			elf_memory(elf_file) + phdr->p_offset;
	}
